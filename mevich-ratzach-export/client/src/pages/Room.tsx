...
const DEFAULT_PACKS = ["classic", "israeli", "trash", "dating", "family"];
...
export default function Room() {
  ...
  async function submitCard() {
    ...
    const sub: Submission = { id: uid(), playerId: me.id, playerName: me.name, card: selectedCard, votes: 0 };
    let removed = false;
    const updatedMe = { ...me, hand: me.hand.filter((c) => { if (!removed && c === selectedCard) { removed = true; return false; } return true; }) };
  ...
  return (
    <div className="min-h-screen bg-black text-white" style={{ fontFamily: "'Heebo', sans-serif", direction: "rtl" }}>
      ...
      <div className="max-w-5xl mx-auto px-4 pt-2 text-left text-[10px] text-white/20" dir="ltr">sync {Math.floor((Date.now() - lastSync) / 1000)}s · {hasSupabase ? "supabase" : "local"}</div>

      {roomState.phase === "lobby" && (
        <main className="max-w-3xl mx-auto px-4 py-10">
          <div className="text-center mb-8"><h1 className="font-black text-5xl mb-3">לובי</h1><p className="text-white/40 font-bold">{hasSupabase ? "Realtime מחובר + רענון אוטומטי" : "מצב מקומי — Supabase לא מחובר"}</p></div>
