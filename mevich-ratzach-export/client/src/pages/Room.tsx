...
const DEFAULT_PACKS = ["classic", "israeli", "office"];
...
function emptyState(packs = DEFAULT_PACKS): RoomState {
  const safePacks = packs.filter((p) => PACKS[p]);
  return {
    phase: "lobby",
    round: 0,
    judgeIndex: 0,
    ...
    submissions: [],
    winner: null,
    selectedPacks: safePacks.length ? safePacks : DEFAULT_PACKS,
    event: null,
    deck: shuffle(allAnswers(safePacks.length ? safePacks : DEFAULT_PACKS)),
    questionDeck: shuffle(allQuestions(safePacks.length ? safePacks : DEFAULT_PACKS)),
    scoreLimit: 7,
  };
}
...
function rowsToPlayers(rows: any[]): Player[] {
  return (rows || []).map((r: any) => ({
    ...
    hand: Array.isArray(r.hand) ? r.hand : [],
...
export default function Room() {
  ...
  async function saveRoom(next: RoomState) {
    ...
    if (!hasSupabase || !supabase) {
      localStorage.setItem(`mevich_room_state_${roomId}`, JSON.stringify(next));
      localStorage.setItem(`mevich_room_players_${roomId}`, JSON.stringify(players));
      return;
    }
  ...
  async function savePlayers(nextPlayers: Player[]) {
    ...
    if (!hasSupabase || !supabase) {
      localStorage.setItem(`mevich_room_players_${roomId}`, JSON.stringify(nextPlayers));
      return;
    }
  ...
  async function pullRemote() {
    ...
    const { data: playerRows, error: playersError } = await supabase.from("players").select("*").eq("room_id", roomId).order("created_at");
    ...
    if (room?.game_state && Object.keys(room.game_state).length) {
      setRoomState(room.game_state as RoomState);
    }
  ...
  useEffect(() => {
        ...
        if (hasSupabase && supabase) {
          ...
          if (!existing) {
            ...
            await supabase.from("rooms").insert({
              ...
              selected_packs: initState.selectedPacks,
              secrets: [],
            });
          }
          ...
          const savedState = localStorage.getItem(`mevich_room_state_${roomId}`);
          const savedPlayers = localStorage.getItem(`mevich_room_players_${roomId}`);
          if (!savedState) localStorage.setItem(`mevich_room_state_${roomId}`, JSON.stringify(emptyState(initialPacks)));
          if (!savedPlayers) localStorage.setItem(`mevich_room_players_${roomId}`, JSON.stringify([host]));
        }
    ...
    if (!hasSupabase || !supabase) return () => { mounted = false; };
  ...
  function drawCards(currentPlayers: Player[], currentState: RoomState) {
    ...
    const nextPlayers = currentPlayers.map((p) => ({ ...p, hand: [...p.hand] }));

    for (const p of nextPlayers) {
      while (p.hand.length < 7) {
        if (deck.length === 0) deck = shuffle(allAnswers(currentState.selectedPacks));
  ...
  async function startGame() {
    ...
    await saveRoom({ ...base, deck });
  }

  async function submitCard() {
    if (!me || isJudge || hasSubmitted || !selectedCard) return;
    ...
    const updatedMe = { ...me, hand: me.hand.filter((c) => c !== selectedCard) };
  ...
  async function resetGame() {
    ...
    await savePlayers([{ ...host, score: 0, hand: [], wins: 0 }]);
    await saveRoom(nextState);
  }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center" dir="rtl">
        <div className="font-black text-xl">טוען חדר...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-black text-white" style={{ fontFamily: "'Heebo', sans-serif", direction: "rtl" }}>
      ...
      <nav className="border-b border-white/10 px-4 py-3 flex items-center justify-between sticky top-0 z-50" style={{ background: "rgba(0,0,0,0.95)", backdropFilter: "blur(12px)" }}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="border border-white/20 rounded px-3 py-1 font-black text-lg tracking-widest">{roomId}</div>
        </div>
        <button onClick={copyCode} className="text-xs font-black px-3 py-1.5 border border-white/20 rounded transition-all" style={{ background: copyFeedback ? "white" : "transparent", color: copyFeedback ? "black" : "rgba(255,255,255,0.6)" }}>
      ...
      {roomState.phase === "lobby" && (
        <main className="max-w-3xl mx-auto px-4 py-12">
          <div className="text-center mb-10">
            ...
            <p className="text-white/40 font-bold">{hasSupabase ? "Realtime מחובר" : "מצב מקומי — Supabase לא מחובר"}</p>
          </div>
          ...
          <div className="border-2 border-white/20 rounded-2xl p-8 text-center mb-8" style={{ background: "#0d0d0d" }}>
            ...
            <div className="font-black text-6xl tracking-widest mb-4">{roomId}</div>
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-black text-base">שחקנים ({players.length})</h2>
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              {roomState.selectedPacks.map((id) => (
          ...
          {isHost ? (
            <button onClick={startGame} disabled={players.length < 2} className="btn-primary w-full justify-center text-lg py-4">🎮 התחל משחק ({players.length} שחקנים)</button>
          ) : (
      ...
      {(roomState.phase === "choose" || roomState.phase === "vote" || roomState.phase === "reveal") && (
        <main className="max-w-5xl mx-auto px-4 py-6">
          <div className="flex items-center gap-3 mb-6 overflow-x-auto pb-1">
            {players.map((p, i) => (
              <div key={p.id} className="flex-shrink-0 flex items-center gap-2 border rounded-full px-3 py-1.5" style={{ borderColor: i === roomState.judgeIndex ? "white" : "rgba(255,255,255,0.15)", background: i === roomState.judgeIndex ? "rgba(255,255,255,0.08)" : "transparent" }}>
                {i === roomState.judgeIndex && <span className="text-xs">⚖️</span>}
          ...
          <div className="mb-8">
            <div className="text-xs font-black text-white/30 mb-3 tracking-widest">סיבוב {roomState.round} · שופט: {judge?.name}</div>
            <div className="game-card-black" style={{ maxWidth: 520, minHeight: 180 }}>
              ...
              <div className="font-black text-white text-xl leading-tight flex-1">{roomState.currentQuestion}</div>
          ...
          {roomState.phase === "choose" && (
            <section>
              {isJudge ? (
                <div className="text-center py-8"><div className="text-4xl mb-4">⚖️</div><div className="font-black text-xl mb-2">אתה השופט הסיבוב הזה</div><div className="text-white/40 font-bold">ממתין לתשובות...</div><div className="mt-4 text-sm text-white/30 font-bold">{roomState.submissions.length} / {players.length - 1} תשובות התקבלו</div></div>
              ) : hasSubmitted ? (
                <div className="text-center py-8"><div className="text-4xl mb-4">✅</div><div className="font-black text-xl mb-2">התשובה שלך נשלחה</div><div className="text-white/40 font-bold">ממתין לשאר...</div></div>
              ) : (
                <div>
                  <div className="flex items-center justify-between mb-4"><h2 className="font-black text-lg">בחר קלף</h2><div className="text-sm text-white/40 font-black">🃏 {me?.hand.length || 0} קלפים ביד</div></div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
                    {me?.hand.map((card, idx) => <button key={card + idx} onClick={() => setSelectedCard(card)} className={`answer-card ${selectedCard === card ? "selected" : ""}`}>{card}</button>)}
                  </div>
              ...
              {isJudge ? (
                <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {roomState.submissions.map((sub) => <button key={sub.id} onClick={() => pickWinner(sub)} className="answer-card">{sub.card}</button>)}
                  </div>
                </>
              ) : (
                <div className="text-center py-10"><div className="text-4xl mb-4">👀</div><div className="font-black text-xl">ממתין לשופט לבחור...</div></div>
              )}
            </section>
          )}

          {roomState.phase === "reveal" && roomState.winner && (
            <section className="text-center">
              ...
              <div className="game-card-white mx-auto mb-6" style={{ maxWidth: 520 }}>
                <div className="font-black text-black text-xl">{roomState.winner.card}</div>
                <div className="mt-6 text-black/50 font-black">— {roomState.winner.playerName}</div>
              </div>
              <div className="roast-box mb-6">🤖 {ROASTS[Math.floor(Math.random() * ROASTS.length)]}</div>
              {canControl ? <button onClick={nextRound} className="btn-primary px-10 py-4">סיבוב הבא</button> : <div className="text-white/40 font-bold">ממתין לסיבוב הבא...</div>}
            </section>
          )}
        </main>
      )}
      ...
      {roomState.phase === "end" && (
        <main className="max-w-3xl mx-auto px-4 py-12 text-center">
          ...
          <div className="grid grid-cols-2 gap-3 mb-8">
            {players.map((p) => <div key={p.id} className="border border-white/10 rounded-xl p-4" style={{ background: "#0d0d0d" }}><div className="font-black">{p.name}</div><div className="text-white/40 text-sm">{titleForPlayer(p)}</div><div className="score-badge mx-auto mt-2">{p.score}</div></div>)}
          </div>
